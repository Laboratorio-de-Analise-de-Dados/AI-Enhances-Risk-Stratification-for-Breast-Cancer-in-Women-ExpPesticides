...
class ModelosAlgoritmosCorrelacao:
    ...
    def selecao_correlacao(self, dados):
    
        # Calculating the correlation matrix
        matrix_correlacao = dados.corr()

        # Plotting the heatmap with selected correlations
        plt.figure(figsize=(17, 15))
        sn.heatmap(matrix_correlacao, annot=True, fmt=".1f")

        # Saving the figure
        plt.savefig('heatmap_correlacao.png')

        # Applying the correlation threshold filter
        resultados = matrix_correlacao.applymap(lambda x: True if -0.3 <= x <= 0.3 else False)
        soma_resultados = resultados.sum()

        # Determining the number of variables analyzed
        total_variaveis = len(matrix_correlacao)

        # Calculating 70% of the number of analyzed variables
        valor_selecao = 0.7 * total_variaveis

        # Selecting only the variables with True return
        variaveis_selecionadas = soma_resultados[soma_resultados > valor_selecao].index.tolist()
    ...
    def estrutura_estratifica(self):
        ...
        df_3 = df_2.dropna()
        variaveis_selecionadas = self.selecao_correlacao(df_3)
        df_4 = df_3[variaveis_selecionadas]
        
        if self.predicao not in df_4.columns:
            df_4[self.predicao] = self.df.loc[df_4.index, self.predicao]
        
        
        if self.oversample == 'Sim':
            # Separating the features and the target variable
            X = df_4.drop("Risk stratification", axis=1)
            y = df_4["Risk stratification"]
            ...
            df_3 = df_2.dropna()
            variaveis_selecionadas = self.selecao_correlacao(df_3)
            dados = df_3[variaveis_selecionadas]
            
            if self.predicao not in dados.columns:
                dados[self.predicao] = self.df.loc[dados.index, self.predicao]
    ...
    def estrutura_exposicao(self):
        ...
        if 'Risk stratification' in df_4.columns:
            df_4['Risk stratification'].replace('Medium', 2, inplace=True)
            df_4['Risk stratification'].replace('High', 3, inplace=True)
            df_4['Risk stratification'].replace('Low', 1, inplace=True)

        
        
        if self.oversample == 'Sim':
            # Separating the features and the target variable
            X = df_4.drop("Exposure to pesticides", axis=1)
            y = df_4["Exposure to pesticides"]
            ...
            valor = df_4['Exposure to pesticides'].value_counts()
            valor_a = valor.tolist()
            menor_valor = min(valor_a)

            classe_0 = df_4[df_4['Exposure to pesticides'] == 0]
            classe_1 = df_4[df_4['Exposure to pesticides'] == 1]
    ...
    def estrutura_pior_prognostico(self):
        ...
        if self.oversample == 'Sim':
            # Separating the features and the target variable
            X = df_4.drop("Worst prognosis", axis=1)
            y = df_4["Worst prognosis"]
            ...
            valor = df_4['Worst prognosis'].value_counts()
            valor_a = valor.tolist()
            menor_valor = min(valor_a)

            # Splitting the DataFrame into two classes
            classe_0 = df_4[df_4['Worst prognosis'] == 0]
            classe_1 = df_4[df_4['Worst prognosis'] == 1]
    ...
    def informacoes_df(self, dados):
        ...
        variaveis_selecionadas = dados.columns
        informacoes = [tamanho, divisao_classes, variaveis_selecionadas]
